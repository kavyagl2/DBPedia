...
class EncoderStack(tf.keras.layers.Layer):
  ...
  def call(self, node_tensor, label_tensor, node1_tensor, node2_tensor,
           attention_bias, inputs_padding, training=False):
...
class TransGAT(tf.keras.Model):
  ...
  def call(self, nodes, labels, node1, node2, targ=None, mask=None, training=False):
    """
    ...
    :param node1: node1 features for edges
    :type node1: tf.tensor
    :param node2: node2 features for edges
    ...
    if targ is not None:
        ...
        predictions = self.predict(enc_output, attention_bias, training)
        return predictions
